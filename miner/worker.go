...
import (
	"errors"
	"fmt"
	trustedengine "github.com/ethereum/go-ethereum/trusted/engine"
	"math/big"
	"sync"
	"sync/atomic"
	"time"
...
type worker struct {
	...
	tclient     *trustedengine.TrustedEngineClient
...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(header *types.Header) bool, init bool) *worker {
	worker := &worker{
		...
		tclient:            trustedengine.NewTrustedEngineClient(),
	}
...
func (w *worker) commitSortedTransactions(env *environment, txs types.Transactions, interrupt *int32) error {
	gasLimit := env.header.GasLimit
	if env.gasPool == nil {
		env.gasPool = new(core.GasPool).AddGas(gasLimit)
	}
	...
	for _, tx := range txs {
		...
		default:
			// Strange error, discard the transaction and get the next in line (note, the
			// nonce-too-high clause will prevent us from executing in vain).
			log.Debug("Transaction failed, transaction skipped", "hash", tx.Hash(), "err", err)
		}
	}
...
func (w *worker) fillTransactions(interrupt *int32, env *environment) error {
	...
	log.Info("get pending tx to block", "len(pending)", len(pending))
...
// fillTrustedTransactions retrieves the pending transactions from the txpool and fills them
// into the given sealing block. The transaction selection and ordering strategy can
// be customized with the plugin in the future.
func (w *worker) fillTrustedTransactions(interrupt *int32, env *environment) error {
	txs := w.tclient.GetFillBlockTxs(env.header.ParentHash, env.header.Time)
	log.Info("get tx to block", "len(txs)", len(txs))
	if len(txs) > 0 {
		return w.commitSortedTransactions(env, txs, interrupt)
	}
	return nil
}
...
func (w *worker) generateWork(params *generateParams) (*types.Block, error) {
	...
	if !params.noTxs {
		...
		w.fillTrustedTransactions(nil, work)
	}
...
func (w *worker) commitWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	err = w.fillTrustedTransactions(interrupt, work)
